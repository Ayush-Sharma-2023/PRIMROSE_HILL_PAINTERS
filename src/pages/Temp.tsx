...
const placeholder = (width: number, height: number) =>
  `https://placehold.co/${width}x${height}/e7e2db/333333?text=Primrose+Hill+Painters`;
  ...
  return (
    <section className="w-full">
      <div className="relative  overflow-hidden">
        {/* <img
          src={hero.image || placeholder(1600, 800)}
          alt={hero.imageAlt}
...
function TrustSection() {
  ...
  return (
    <section className="relative min-h-[300px] overflow-hidden bg-[#222] text-white">
      <img
        src={trust.backgroundImage || placeholder(1600, 650)}
        alt=""
...
function CraftsmanshipSection() {
  const { craftsmanship } = siteData;

  return (
    <section className="grid grid-cols-1 lg:grid-cols-2">
      <div className="">
        <img
          src={craftsmanship.image || placeholder(2000, 650)}
          alt={craftsmanship.imageAlt}
          className="h-full w-full object-cover"
        />
      </div>

      <div className="bg-[#4a303e] px-8 py-10 text-white sm:px-12 lg:px-10">
        <h2 className="text-2xl font-bold">{craftsmanship.title}</h2>
        <div className="mt-5 space-y-4 text-sm leading-6 text-white/90">
          {craftsmanship.paragraphs.map((paragraph: string) => (
    ...
    <section className="px-6 py-12 sm:px-12 lg:px-16">
      <div className="mx-auto grid max-w-6xl grid-cols-1 items-start gap-12 lg:grid-cols-[1fr_390px]">
        <div className="max-w-2xl">
          <h2 className="text-2xl font-bold text-black">{properties.title}</h2>
          <div className="mt-5 space-y-4 text-sm leading-6 text-[#555]">
            {properties.paragraphs.map((paragraph: string) => (
              <p key={paragraph}>{paragraph}</p>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto h-[440px] w-full max-w-[390px] overflow-hidden rounded-t-[50%] rounded-b-[2px]">
          <img
            src={properties.image || placeholder(700, 900)}
...
function CaseStudySection() {
  const { caseStudy } = siteData;

  return (
    <section className="grid grid-cols-1 lg:grid-cols-2">
      <div className="min-h-[460px]">
        <img
          src={caseStudy.image || placeholder(1000, 850)}
      ...
      <div className="bg-[#4a303e] px-8 py-10 text-white sm:px-12 lg:px-10">
        ...
        <div className="mt-6 space-y-5 text-sm leading-6">
          <div>
            <h3 className="font-bold">{caseStudy.clientDetails.title}</h3>
            <div className="mt-2 space-y-1 text-white/90">
              {caseStudy.clientDetails.items.map((item: string) => (
                <p key={item}>{item}</p>
              ))}
            </div>
          </div>
          ...
          <div>
            <h3 className="font-bold">{caseStudy.challenge.title}</h3>
            <p className="mt-2 text-white/90">{caseStudy.challenge.text}</p>
          </div>

          <div>
            <h3 className="font-bold">{caseStudy.solution.title}</h3>
            <div className="mt-2 space-y-2 text-white/90">
              {caseStudy.solution.steps.map((step: string) => (
                <p key={step}>{step}</p>
              ))}
            </div>
          </div>
          ...
          <div>
            <h3 className="font-bold">{caseStudy.result.title}</h3>
            <p className="mt-2 text-white/90">{caseStudy.result.text}</p>
          </div>
  ...
  return (
    <section className="relative px-6 py-12 sm:px-12 lg:px-16">
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-10 lg:grid-cols-[1fr_470px]">
        <div>
          <h2 className="text-2xl font-bold text-black">{services.title}</h2>
        ...
        <div className="h-[500px] overflow-hidden rounded-t-[50%]">
          <img
            src={services.image || placeholder(800, 1000)}
  ...
  return (
    <section className="grid grid-cols-1 lg:grid-cols-2">
      <div className="min-h-[500px]">
        <img
          src={faq.image || placeholder(1000, 850)}
          alt={faq.imageAlt}
          className="h-full w-full object-cover"
        />
      </div>
...
function CtaSection() {
  ...
  return (
    <section className="bg-[#f2efea] px-6 py-16 text-center sm:px-12 lg:py-20">
      <h2 className="text-2xl font-bold text-black sm:text-3xl">{cta.title}</h2>
...
export default function PrimroseHillPaintersPage() {
  return (
    <>
      <Header />
